...
path, ~0.3 ms/cycle), Newest/Name/Release sort, windowed grid,
...
src/
  ...
  data/
    tag-taxonomy.json           # COMMITTED, human-maintained: tag aliases + dropdown groups (filter-redesign-spec D3)
    filter-index.example.json   # COMMITTED shape doc (hand-written)
    filter-index.json / details.json  # generated; committed snapshot
scripts/
  ...
  lib/ { recency.cjs, model-resolve.mjs, thumbnail-paths.cjs, tag-taxonomy.mjs }
...
| `scripts/lib/tag-taxonomy.mjs` | 90% | met (pure; load/validate/applyAliases/orderTags/assignGroups) |
| `scripts/build-filter-index.mjs` -- `buildIndex()` | 85% | met (`main()` fs glue exempt) |
| `src/components/GalleryIsland.tsx` | 70% | met (drawer/chip/group interaction tests) |
